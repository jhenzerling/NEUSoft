...
from Modules.MNIST import Loader_MNIST as L
...
def setSummaries(sess,costfunction,accuracy,trainbatch,number):
	...
	trim  = tf.reshape(trainbatch[0],[CM.TRAINING['Training Batch Size'],28,28,1])
...
def setFeeds(a,b,c,d,kp):
	Qtestbatch0,Qtestbatch1 = a,b
...
def Training(dobj,nobj):
	f					= nobj.getF()
	[x,y]					= nobj.getIOT()
	[trainbatch,testbatch]			= L.batcher()
		...
		if graphtrigger == True:
			...
			for i in range(stepnumber):
				with tf.variable_scope('Feeding'):
					trainfeed = setFeeds(trainbatch[0],trainbatch[1],f,y,keep_prob)
					testfeed = setFeeds(testbatch[0],testbatch[1],f,y,keep_prob)
				...
				if i % saverate == 0:
					x2 = 1
					ssf_path = saver.save(sess, 'Output/Weights/MNIST',global_step=i)
